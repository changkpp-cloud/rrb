...
### บอร์ดรำลึกขึ้นหน้าแรก (รวมทุกงาน) + สปอนเซอร์ถอยเมื่อผู้มอบเยอะ — 2026-06-30
- ต่อยอดจากบอร์ด `DynamicVirtualBoard` ที่ทีมทำไว้บนหน้างาน → **นำมาแสดงบนหน้าแรกด้วย** (รวมผู้มอบจากทุกงาน) ผ่าน `getRecentBoardDonations()` ใน lib/memorial
- แก้ลำดับป้ายใน `toBoardTags`: เดิมสปอนเซอร์ขึ้นก่อน → เปลี่ยนเป็น **ผู้มอบจริงขึ้นก่อน แล้วเติมช่องที่เหลือด้วยสปอนเซอร์** (CAP 8) → พอผู้มอบจริงเยอะขึ้นสปอนเซอร์ถูกดันออกเองจนหมด (มีผลทั้งบอร์ดหน้าแรกและหน้างาน)
- แสดงเฉพาะ ชื่อ+ตำแหน่ง+ข้อความหลังป้าย (ไม่โชว์ยอด/เบอร์) · หมายเหตุ: การกดยังเป็น modal ตามของเดิม (flip เป็นออปชันเพิ่มได้ภายหลัง)

### เอกสารบริบทสำหรับ Claude.ai Project — 2026-07-02
- สร้าง `docs/project-context/` 5 ไฟล์ (อ่านจากโค้ดจริง): PROJECT-OVERVIEW.md (ภาพรวม+stack+roles+workflow), STRUCTURE.md (โครงสร้างไฟล์), DATABASE-SCHEMA.md (ทุกตาราง+ความสัมพันธ์+RLS+enums), API-ROUTES.md (~48 endpoints+สิทธิ์), CURRENT-STATUS.md (เสร็จ/ค้าง/known issues/แผน)
- CLAUDE.md ที่ root มีอยู่แล้ว — ไม่สร้างซ้ำ
