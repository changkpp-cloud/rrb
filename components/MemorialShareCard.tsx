...
// ── Share card: QR + ลิงก์หน้างาน (ผู้มอบ) ────────────────────────────────────
export default function MemorialShareCard({ publicUrl, slug }: Props) {
  ...
  return (
    <div className="bg-cream-50 rounded-2xl gold-border card-shadow px-5 py-5 space-y-4">
      <div className="text-center">
        <p className="text-xs font-semibold text-gold-600">QR Code + ลิงก์หน้างาน</p>
        <p className="text-[10px] text-gold-400 mt-0.5">ให้ผู้มอบสแกนเพื่อเข้าหน้าผู้วายชนม์และร่วมบุญ</p>
      </div>

      <QrImage url={publicUrl} filename={`qr-${slug}.png`} />

      <div className="bg-white rounded-xl border border-gold-200 px-3 py-2.5">
        <p className="text-xs text-gold-700 break-all font-mono">{publicUrl}</p>
      </div>
