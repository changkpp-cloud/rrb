...
```
/                          → Landing page (ลิสต์งานที่เปิดอยู่ + บอร์ดรำลึกผู้ร่วมบุญรวมทุกงาน `DynamicVirtualBoard` ผ่าน `getRecentBoardDonations`) — ไม่ใช่ redirect
...
/privacy                   → นโยบายความเป็นส่วนตัว (PDPA) — static, ลิงก์จาก SiteFooter
```
...
/dashboard/admin/audit     → บันทึกกิจกรรม (Audit Log) เรื่องเงิน: แก้บัญชี/ยืนยันเบอร์ · ปิดงาน · เก็บค่าดำเนินการ (อ่านจาก audit_logs)
